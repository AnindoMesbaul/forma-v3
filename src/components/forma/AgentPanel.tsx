...
export function AgentPanel() {
  ...
  return (
    <aside className="flex h-full w-[344px] shrink-0 flex-col border-r border-border bg-surface shadow-[var(--shadow-panel)]">
      {aiThinking && (
      ...
      <div className="flex shrink-0 border-b border-border">
        <TabButton active={tab === "proposals"} onClick={() => setTab("proposals")}>
          Proposals
          {proposals.length > 0 && (
            <span className="ml-1.5 rounded-[4px] bg-surface-2 px-1 py-px text-[11px] tabular-nums text-muted-foreground">
              {proposals.length}
            </span>
          )}
        </TabButton>
...
function TabButton({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex flex-1 items-center justify-center gap-1 border-b-2 px-3 py-2.5 text-sm font-medium transition-colors ${
        active
